...
Provenance: this module comes from the dictate.sh project by Marc Puig
(MIT, https://github.com/mpuig/dictate.sh) and serves as parlando's ASR
engine. Local modifications: tagged partial streaming (--emit-partials),
energy gating against silence hallucination, safe peak normalization
(--normalize), and error hardening in the processing loop.
...
from parlando import webrtcvad_compat as webrtcvad  # noqa: E402
...
class RealtimeTranscriber:
    ...
    def __init__(
        ...
        self.device = device
        self.no_ui = no_ui
        # Stream partial transcripts (tagged) to stdout for live dictation.
        self.emit_partials = emit_partials
        # Boost low-level mic input before ASR (safe, capped).
        self.normalize = normalize
        # Min RMS (normalized) of recent audio to run ASR; blocks silence
        # hallucinations. Lower = more sensitive to soft speech.
    ...
    def _has_speech_energy(self, audio_int16: np.ndarray) -> bool:
        """Cheap energy gate: is there actual sound right now?

        More reliable than webrtcvad's binary state for deciding whether to
        run ASR, which prevents silence-driven hallucinations.
        """
        if audio_int16.size == 0:
            return False
        audio = audio_int16.astype(np.float32) / 32768.0
        rms = float(np.sqrt(np.mean(np.square(audio))))
        return rms >= self.energy_floor

    def _normalize_audio(self, audio: np.ndarray) -> np.ndarray:
        """Peak-normalize a float window for low-level mics.

        Safe by construction: skips near-silence so background noise is not
        amplified, caps the gain, and clips to [-1, 1] so we never introduce
        distortion the model was not trained on.
        """
        if audio.size == 0:
            return audio
        peak = float(np.max(np.abs(audio)))
        if peak < 0.03:  # Near-silence/noise floor: do not boost.
            return audio
        gain = min(0.3 / peak, 8.0)  # Target ~0.3 peak, cap amplification.
        if gain <= 1.0:
            return audio
        ...
        while True:
            ...
            if frame is not None:
                ...
                if turn_complete:
                    try:
                        ...
                        # One bad turn must not kill the pipeline task.
                        LOGGER.exception("Turn finalize failed; continuing")
            ...
            if now - last_transcribe >= self.transcribe_interval:
                if (
                    ...
                    async with self.buffer_lock:
                        recent_int16 = self._get_recent_audio(0.5)
                        audio_int16 = self._get_recent_audio(self.max_buffer_seconds)

                    # Gate ASR on current sound energy, not webrtcvad's flaky
                    # binary state. Transcribing silence makes Whisper-style
                    # models hallucinate fluent, fabricated text.
                    if not self._has_speech_energy(recent_int16):
                        last_transcribe = now
                        self.last_transcribed_sample = self.total_samples_written
                        continue
                    ...
                    if audio_int16.size >= int(self.sample_rate * 0.3):
                        if not self.gpu_lock.locked():
                            try:
                                ...
                                # One bad inference must not kill the task.
                                LOGGER.exception("ASR update failed; continuing")
                            self.ui_status = "Listening"
                        self.last_transcribed_sample = self.total_samples_written
        ...
        while True:
            ...
            if self.pending_analysis:
                ...
                if not is_tty:
                    ...
                    # In partial-streaming mode, tag final turns so a downstream
                    # dictation client can distinguish commits from partials.
                    prefix = "F\t" if self.emit_partials else ""
                    os.write(1, f"{prefix}{transcript}\n".encode())
                elif not self.live:
            ...
            if self.current_transcript and self._is_meaningful(self.current_transcript):
                if self.current_transcript != last_displayed and not self.turn_complete:
                    ...
                    if not is_tty:
                        # Stream partials (tagged) for live dictation clients.
                        if self.emit_partials:
                            os.write(
                                1, f"P\t{self.current_transcript}\n".encode()
                            )
                    elif not self.live:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--emit-partials",
        action="store_true",
        help="When piped, stream tagged partial (P) and final (F) lines for live dictation",
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
        help="Boost low-level mic input before ASR (safe peak-normalize, capped)",
    )
    parser.add_argument(
        "--energy-floor",
        type=float,
        default=0.004,
        help="Min normalized RMS to run ASR; blocks silence hallucination "
             "(lower = more sensitive, default: 0.004)",
    )
    parser.add_argument(
